...
const router = Router();
const body = (value: z.ZodType) =>
  validate(z.object({ body: value, params: z.any(), query: z.any() }));
...
const toolInput = z.object({
  ...
  fullDescription: z.string().min(50).max(10000),
  category: z.enum([
    "Content",
    "Chat",
    "Data",
...
router.post(
  "/auth/register",
  body(credentials.extend({ name: z.string().min(2).max(80) })),
...
router.post(
  ...
  authenticate,
  body(toolInput),
  asyncHandler(tools.createTool),
);
router.patch(
  "/tools/:id",
  authenticate,
  body(toolInput.partial()),
  asyncHandler(tools.updateTool),
);
...
router.put(
  ...
  asyncHandler(social.upsertReview),
);
router.delete(
  "/reviews/item/:id",
  authenticate,
  asyncHandler(social.deleteReview),
);
router.get("/favorites", authenticate, asyncHandler(social.listFavorites));
router.post(
  "/favorites/:toolId",
  authenticate,
  asyncHandler(social.addFavorite),
);
router.delete(
  "/favorites/:toolId",
  authenticate,
  asyncHandler(social.removeFavorite),
);
...
router.post(
  "/conversations",
  authenticate,
  body(z.object({ title: z.string().max(100).optional() })),
  asyncHandler(ai.createConversation),
);
router.patch(
  "/conversations/:id",
  authenticate,
  body(z.object({ title: z.string().min(1).max(100) })),
  asyncHandler(ai.updateConversation),
);
router.delete(
  "/conversations/:id",
  authenticate,
  asyncHandler(ai.deleteConversation),
);
router.get(
  "/conversations/:id/messages",
  authenticate,
  asyncHandler(ai.getMessages),
);
router.post(
  "/conversations/:id/messages",
  authenticate,
  body(z.object({ content: z.string().min(1).max(10000) })),
  asyncHandler(ai.sendMessage),
);
...
router.post(
  ...
  body(
    z.object({
      ...
      budget: z.string(),
      outputPreference: z.string(),
      refinement: z.string().optional(),
    }),
  ),
  asyncHandler(ai.recommend),
);
router.post(
  ...
  body(
    z.object({
      toolId: z.string(),
      feedback: z.enum(["helpful", "not-helpful", "saved"]),
    }),
  ),
  asyncHandler(ai.recommendationFeedback),
);
router.post(
  "/content/generate",
  authenticate,
  ...
  asyncHandler(ai.generateContent),
);
router.get("/content", authenticate, asyncHandler(ai.listGenerations));

router.get("/images/status", authenticate, asyncHandler(images.status));
router.post(
  "/images/generate",
  authenticate,
  body(
...
router.post(
  ...
  authenticate,
  upload.single("file"),
  asyncHandler(ai.analyzeDocument),
);
...
router.patch(
  ...
  asyncHandler(misc.updateProfile),
);
router.get(
  "/users",
  authenticate,
  requireRole("admin"),
  asyncHandler(misc.listUsers),
);
router.post(
  "/contact",
  body(
