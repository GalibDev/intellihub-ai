...
function assertSafeKeys(value: unknown): void {
  ...
  for (const [key, nested] of Object.entries(value)) {
    if (key.startsWith("$") || key.includes(".")) throw new ApiError(400, "Request contains an unsafe field name");
...
app.use((req, _res, next) => { try { assertSafeKeys(req.body); assertSafeKeys(req.params); next(); } catch (error) { next(error); } });
app.get("/health", (_req, res) => res.json({ success: true, message: "IntelliHub API is healthy", data: { timestamp: new Date().toISOString() } }));
app.use("/api", routes);
app.use(notFound);
